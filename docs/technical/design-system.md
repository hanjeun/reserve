...
## 목록을 넘기는 방법 — 무한 스크롤 vs 페이지네이션 (2026-09-03 등재)

"모바일은 무한 스크롤, PC는 페이지네이션"으로 나눈 게 아니다. **화면의 성격**으로 나눈다.

| 성격 | 화면 | 방식 | 구현 |
|------|------|------|------|
| **탐색** — 마음에 드는 걸 만날 때까지 훑는다 | 가게 목록(`/stores`) | 무한 스크롤 | `useStoreList.js` (`useInfiniteQuery`) |
...
### 구현 상태 (2026-09-09 dev 기준, production 미배포)
...
정상 0건과 조회 실패는 다른 화면 상태다. PR #195의 로컬 mock/H2와 GitHub CI 검증을 통과했지만
아직 production 반영 증거는 아니다.
...
탐색에는 "3페이지"라는 좌표가 아무 의미가 없다. 사용자는 순위를 세는 게 아니라 눈에 들어오는 걸
찾는 중이고, 스크롤이 끊기면 그 흐름이 끊긴다.

관리는 정반대다. "어제 처리하던 게 42페이지였다", "이 목록은 총 몇 건이다", "이 링크를 그대로
동료에게 보낸다" 가 전부 필요하다. 무한 스크롤에는 그 좌표가 없다 — 새로고침하면 처음으로
돌아가고, 500번째 행을 다시 보려면 매번 처음부터 다시 긁어야 한다.
그래서 관리 화면은 **모바일에서도 페이지네이션**이다. 화면 크기가 아니라 하는 일이 기준이다.
(모바일에서 버튼이 줄바꿈되는 문제는 `DataTable` 이 `showLessItems` + `size="small"` 로 따로 해결한다)

### 새 목록을 만들 때

- 사용자가 **고르는** 목록인가 → 무한 스크롤. 총 개수·페이지 번호를 노출하지 않는다.
