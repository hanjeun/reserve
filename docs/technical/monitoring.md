...
> **2026-09-06 운영 상태:** 검증 스크립트에서 로그 9/10, 서버 자원 5/6 쿼리 예산으로 통과한
> 두 JSON을 운영 Grafana에 overwrite import했다. 실제 화면에서 stat 카드의 내부 쿼리명 제거,
> 접힌 장기 범위 패널, 서버 자원 수치와 컨테이너 표를 확인했다. 따라서 새 디자인과 쿼리 예산은
> 운영 반영 완료 상태다. 다만 Discord contact point의 실제 수신과 개별 alert rule 발화는 별도 증거다.
