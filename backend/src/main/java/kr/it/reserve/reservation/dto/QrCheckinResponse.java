 ...
 *
 * <p><b>왜 {@link ReservationResponse} 를 그대로 안 쓰나</b> — 화면이 "방금 승인했다"와
 * "이미 승인돼 있었다"를 구분해서 보여줘야 하는데, 예약 응답만으로는 알 수 없다.
 * 체크인은 <b>멱등</b>이라 같은 QR 을 두 번 스캔해도 성공하고, 그때 돌아오는 예약 상태는
 * 두 경우 모두 {@code CONFIRMED} 로 똑같다.
 *
...
public class QrCheckinResponse {
    ...
    /** {@code true} 면 이 스캔 이전에 이미 승인돼 있었다(= 이번 스캔으로 바뀐 게 없다). */
